...
if __name__ == "__main__":
    ...
    for (logbook_uuid, mid), entry in sorted_entries.items():
        ...
        if results:
            ...
            elogy_entry_mod_time = parse_time(get_modification_time(existing_entry))
            imported_entry_mod_time = get_modification_time(entry)

            if (elogy_entry_mod_time >= imported_entry_mod_time):
                # entry has not been edited since import, ignore
                logging.debug("entry {}/{} hasn't been edited, ignoring".format(logbook['name'], mid, get_modification_time(entry)))
                logging.debug("elogy entry mod time: {}".format(elogy_entry_mod_time))
                logging.debug("imported entry mod time: {}".format(imported_entry_mod_time))
